...
public class InternalApiKeyMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        if (context.Request.Path.StartsWithSegments("/api/internal"))
        {
            ...
            {
                _logger.LogWarning("Internal API Key is not configured in Roadmap settings.");
            ...
            {
                _logger.LogWarning("X-Internal-Api-Key header is missing in Roadmap request.");
            ...
            {
                _logger.LogWarning("X-Internal-Api-Key is invalid in Roadmap request.");
