...
class MarketplaceRemoteRepository implements MarketplaceRepository {
  ...
  Future<List<KitchenCardVm>> loadListing({
    ...
    if (filter.nearbyOnly) {
      ...
      partners = await _api.searchPartners(lat: lat, lng: lng, radiusKm: 8, pageSize: 40);

    } else if (filter.macroBalanced) {

      final dishes = await _api.searchFoodMenu(lat: lat, lng: lng, pageSize: 80);

      final balanced = dishes.where(_isMacroBalanced).toList();

      partners = await _partnersFromDishes(balanced, lat: lat, lng: lng);

    } else if (filter.healthyCollection) {

      var dishes = await _api.searchFoodMenu(
      ...
      if (dishes.isEmpty) {
        ...
        dishes = await _api.searchFoodMenu(
      ...
      if (partners.isEmpty) {
        ...
        partners = await _api.searchPartners(lat: lat, lng: lng, pageSize: 40);
      ...
      if (params != null && params['macroBalanced'] == true) {
        ...
        final dishes = await _api.searchFoodMenu(
        ...
        if (partners.isEmpty && category == null && tags == null) {
  ...
  Future<MarketplaceSearchResult> search({
    ...
    final partnersFuture = _api.searchPartners(
    ...
    final dishesFuture = _api.searchFoodMenu(
  ...
  Future<List<Partner>> _partnersFromDishes(
    ...
    if (lat != null && lng != null) {
      ...
      final nearby = await _api.searchPartners(lat: lat, lng: lng, radiusKm: 15, pageSize: 50);
